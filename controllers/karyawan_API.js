   ...
   static async getKaryawan_token (req,res) {
    ...
    static async UpdateKaryawan (req,res) {
        ...
        const query =  Client.query(await query_updateData(nama,deskripsi,ig,posisi,id),(err,result) => {
            if(result.rowCount > 0) res.status(200).json({message:`data  berhasil di ubah`})
            else res.status(400).json({messageError:'ada kesalahan saat ubah data karyawan'})
         })
    }
    ...
    static async deleteKaryawan (req,res) {
       ...
       const getTokenMasterAdmin = Client.query(query_getTokenMasterAdmin(token),(err,result) => {
        if(result.rowCount < 1) { 
            res.status(401).json({message : "token tidak valid dengan token ijin perusahaan. silahkan hubungi admin atau owner"})
       }
        else {
            const query = Client.query(query_deleteData(id,nama),(err,result) => {
                if(result.rowCount > 0) {
                    const delete_akun = Client.query(`DELETE FROM akun_karyawan where id = '${id}' `,(err, result) => {
                        if(result.rowCount > 0){
                            res.status(202).json({message:`data dengan nama ${nama}  berhasil dihapus`})
                        }
                    })
                }
                else {
                    res.status(400).json({message:'ada kesalahan saat menghapus data karyawan. hubungi admin'})
                } 
             })
        }
      })
